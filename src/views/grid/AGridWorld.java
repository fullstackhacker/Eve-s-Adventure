...
public final class AGridWorld extends GridPane {

	private static AGridWorld instant = null;

	public static Label[][] gridButtons = null;
	...
	private AGridWorld() {
		 ...
		 * extends Toggle> ov, Toggle toggle, Toggle new_toggle) { if
		 * (new_toggle == null) { GameTabs.getInstance().enableTab(
		 * GameTabs.OPERATIONS_TAB_VALUE); GameTabs.getInstance().enableTab(
		 * GameTabs.INSTRUCTIONS_TAB_VALUE); GameTabs.getInstance().disableTab(
		 * GameTabs.CREATURES_TAB_VALUE); GameTabs.getInstance().disableTab(
		 * GameTabs.ITEMS_TAB_VALUE); GameTabs.getInstance().switchTab(
		 * GameTabs.OPERATIONS_TAB_VALUE); } else {
		 ...
		 * group.selectedToggleProperty().addListener(new
		 * ChangeListener<Toggle>(){ public void changed(ObservableValue<?
		 * extends Toggle> ov, Toggle toggle, Toggle new_toggle) { if(new_toggle
		 * == null){
		...
		gridButtons = new Label[5][10];

		System.out.println("OVERWERITINEG BUTTONGS");

		for (int i = 0; i < 5; i++) {
			for (int j = 0; j < 10; j++) {
				gridButtons[i][j] = new Label("   ");
	...
	/*
	 * public static int getXCoordinate(){ for(int y = 0; y <
	 * gridButtons.length; y++){ for(int x = 0; x < gridButtons[y].length; x++){
	 * if (AGridWorld.gridButtons[y][x].isSelected()){ System.out.println("X: "
	 * + y); return y; } } } System.out.println("no X coordinate"); return 0; }
	 * 
	 * public static int getYCoordinate(){ for(int y = 0; y <
	 * gridButtons.length; y++){ for(int x = 0; x < gridButtons[y].length; x++){
	 * if (AGridWorld.gridButtons[y][x].isSelected()){ System.out.println("Y: "
	 * + x); return x; } } }
	 * 
	 * System.out.println("no Y coordinate"); return 0; }
	 */

	public static AGridWorld getInstance() {
		return (instant == null) ? instant = new AGridWorld() : instant;
	}
}