...
public class Save {
	...
	private static final String WORLDDIR = Save.DATADIR + "worlds"
			+ File.separator;
	private static final String LEVELDIR = Save.DATADIR + "levels"
			+ File.separator;
	private static final String CAMPAIGNDIR = Save.DATADIR + "campaigns"
			+ File.separator;
	...
	public static boolean saveLevel(Level level, String campaignPath) {
		...
		/* save the world */
		String worldPath = levelPath + File.separator
				+ level.getWorld().getName() + Save.WORLDEXT;
		try {
			fos = new FileOutputStream(new File(worldPath));
			oos = new ObjectOutputStream(fos);
		...
		/* save the karel code */
		String karelCodePath = levelPath + File.separator
				+ Save.KARELCODEFILENAME;
		try {
		...
		String descriptionPath = levelPath + File.separator
				+ Save.DESCRIPTIONFILENAME;
		try {
	...
	public static boolean saveCampaign(Campaign campaign) {
		...
		String campaignPath = Save.CAMPAIGNDIR + campaign.getName()
				+ File.separator;
		FileOutputStream fos = null;
		ObjectOutputStream oos = null;
	...
	public static void main(String args[]) {
		...
		Campaign campaign = new Campaign("My_Campaign",
				"mycampaign description");
		campaign.addLevel(level);

		Campaign campaign2 = new Campaign("c2", "asdfa");
		campaign2
				.addLevel(new Level(new World("asdfasdf", 1, 1), "description"));
